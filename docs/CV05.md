...
- Vyberte si jednu z následujících možností a poté realizujte úkoly. Pokud máte čas, můžete realizovat postupně obě možnosti.
  - Pokud využijete git větve pro různé možnosti, nezapomeňte jednu z nich mergnout do hlavní větve.
...
- Pomocí DI přidejte `UserService` do testovací třídy a opatřete třídu anotací `@MockitoBean`.
